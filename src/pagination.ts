...
export function relayStylePagination<TNode extends Reference = Reference>(
  ...
  return {
    ...
    merge(existing, incoming, { args, isReference, readField }) {
      ...
      //dumpBrief('exiting', existing);
      //dumpBrief('incoming', incoming);
...
function dumpBrief<T>(title: string, relay: TIncomingRelay<T>) {
  console.log(title);
  relay.edges?.forEach((e, i) => {
    console.log(`edge[${i}]: ${e.cursor}`);
  });
  console.log(`pageInfo: ${JSON.stringify(relay.pageInfo, undefined, 2)}`);
}
